...
class ScanToTreeConverter: public TRootanaEventLoop {
    ...
    void BeginRun(int transition,int run,int time){
        ...
        for(int i =0; i < fNChan; i++){  // Save up to 24 waveforms
            ...
            sprintf(descr,"dt5743_wave0%i[num_points][%i]/Double_t",i,num_dt5743_max);
        ...
        if(digitizer_data){      
            ...
            if(num_points<nPoints_max){
                ...
                for(int i = 0; i < measures.size(); i++){           
                    ...
                    if(chan == 0) dt5743_wave0[num_points-1][ib] = measures[i].GetSample(ib);  
                    if(chan == 1) dt5743_wave1[num_points-1][ib] = measures[i].GetSample(ib);  
                    if(chan == 2) dt5743_wave2[num_points-1][ib] = measures[i].GetSample(ib);  
                    if(chan == 3) dt5743_wave3[num_points-1][ib] = measures[i].GetSample(ib);
                    if(chan == 4) dt5743_wave4[num_points-1][ib] = measures[i].GetSample(ib);
                    if(chan == 5) dt5743_wave5[num_points-1][ib] = measures[i].GetSample(ib);
                    if(chan == 6) dt5743_wave6[num_points-1][ib] = measures[i].GetSample(ib);
                    if(chan == 7) dt5743_wave7[num_points-1][ib] = measures[i].GetSample(ib);              
                }   
            } 
